...
static bool transformation_project_internal(const CameraCalibration & calib,
                                            ...
                                            Eigen::Vector4f& J_xy)
{
    ...
    if (!(fx > 0.f && fy > 0.f))
    {
        spdlog::error("Expect both fx and fy are larger than 0, actual values are fx:{0}, fy: {1} for coordinate: ({2},{3})", (double)fx, (double)fy, xy(0), xy(1));
        return false;
    }
...
static bool transformation_iterative_unproject(const CameraCalibration & calib,
...
static bool transformation_unproject_internal(const CameraCalibration& calib,
                                              const Eigen::Vector2f& uv,
                                              Eigen::Vector2f& xy,
                                              int *valid)
{
    ...
    if (!(fx > 0.f && fy > 0.f))
    {
        spdlog::error("DepthtoPointcloudLookupTable: Expect both fx and fy are larger than 0, actual values are fx:{0}, fy: {1} for coordinate: ({2},{3})", (double)fx, (double)fy, xy(0), xy(1));
        return false;
    }
...
bool transformation_unproject(const CameraCalibration& calib,
...
bool createXyLookupTable(const CameraCalibration &calib, cv::Mat &xy_table) {
    if (!(calib.fy > 0.f && calib.fy > 0.f)) {
        spdlog::error("DepthtoPointcloudLookupTable: Expect both fx and fy are larger than 0, actual values are fx:{0}, fy: {1}", (double)calib.fx, (double)calib.fy);
        return false;
    }

    int width = calib.width;
    int height = calib.height;
