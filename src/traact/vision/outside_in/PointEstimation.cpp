...
void PointEstimation::setCountCameras(size_t count) {
    cameras_.resize(count);
}

void PointEstimation::setData(size_t camera_index,
                              const spatial::Pose6D &camera2world,
                              const CameraCalibration &calibration,
                              const KeyPointList &input) {
    cameras_[camera_index].setData(camera2world, calibration, input);
}

void PointEstimation::compute(vision::Position3DList &output_points, std::vector<std::map<size_t,
    ...
    for (int camera_idx = 0; camera_idx < cameras_.size(); ++camera_idx) {
        for (int point_idx = 0; point_idx < cameras_[camera_idx].input_.size(); ++point_idx) {
            ...
            auto &line_1 = cameras_[camera_idx].rays_[point_idx];
            auto &p_1 = cameras_[camera_idx].input_[point_idx];

            for (int camera_idx_2 = camera_idx + 1; camera_idx_2 < cameras_.size(); ++camera_idx_2) {

                for (int point_idx_2 = 0; point_idx_2 < cameras_[camera_idx_2].input_.size(); ++point_idx_2) {
                    auto &line_2 = cameras_[camera_idx_2].rays_[point_idx_2];
                    auto &p_2 = cameras_[camera_idx_2].input_[point_idx_2].pt;
    ...
    for (auto &candidate : candidates) {
        ...
        for (auto &tmp : good_candidates) {
            ...
            image_points[idx] = cameras_[camera_idx].input_[point_idx].pt;
            idx++;
        }
